    ...
    for k in dict_requetes.keys():
        ...
        try :
            ...
            for j in range(len(path)-1):
                u,v = path[j], path[j+1]
                dict_congestion[(u,v)] = dict_requetes[k]
        except Exception:
            pass
...
def print_compare_congestion(G, prepG, dict_requetes):
    ...
    color_list = ox.plot.get_colors(n= vmax + 1, cmap=colormap)
    ec1 = [color_list[d['congestion']] for u, v, k, d in G1.edges(keys=True, data=True)]
    ec2 = [color_list[d['congestion']] for u, v, k, d in G2.edges(keys=True, data=True)]
